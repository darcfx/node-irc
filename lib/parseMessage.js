...
module.exports = function parseMessage(line, stripColors) {
    ...
    if (stripColors) line = ircColors.stripColorsAndStyle(line);
    //logger.info('parseMessage(): ' + line);
    //console.log(line);
    ...
    if (match) {
        ...
        } else message.server = message.prefix;
    }
    ...
    if (!_.isUndefined(trailing) && !_.isEmpty(trailing)) message.args.push(trailing);
