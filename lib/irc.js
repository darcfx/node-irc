...
const logger = require('../../../lib/logger');
 ...
 function Client(server, nick, opt) {
     ...
     self.addListener('raw', message => {
        //if (self.opt.debug === true) logger.info(message); //console.log(JSON.stringify(message));
        handleRaw(message, self)
     });
 ...
 Client.prototype._toLowerCase = function(str) {
     ...
     if (this.supported.casemapping === 'rfc1459') {
         lower = lower.replace(/\[/g, '{').replace(/]/g, '}').replace(/\\/g, '|').replace(/\^/g, '~');
     } else if (this.supported.casemapping === 'strict-rfc1459') {
         lower = lower.replace(/\[/g, '{').replace(/]/g, '}').replace(/\\/g, '|');
     }
     return lower;
 };
