...
const rplChanForward = (message, client) => {
    const [nick, originalChannel, forwardedChannel, dialog] = message.args;
    client.emit('channelForward', {
        nick, originalChannel, forwardedChannel, dialog,
    });
};
...
const cap = (message, client) => {
    if (message.args[0] === '*' &&
            message.args[1] === 'ACK' &&
            message.args[2].startsWith('sasl')) // there's a space after sasl, SOMETIMES
